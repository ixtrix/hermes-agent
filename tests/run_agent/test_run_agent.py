...
class TestRunConversation:
    ...
    def test_prior_turn_tool_tail_does_not_trigger_post_tool_nudge(
        self, agent, monkeypatch
    ):
        """A new user turn must not inherit post-tool recovery from history."""
        self._setup_agent(agent)
        agent.base_url = "http://127.0.0.1:1234/v1"
        empty_resp = _mock_response(content=None, finish_reason="stop")
        content_resp = _mock_response(
            content="Starting the requested browser task.",
            finish_reason="stop",
        )
        ...
        history = [
            ...
            {
                "role": "tool",
                "tool_call_id": "call_prior_2",
                "content": "latest prior result",
            },
            {"role": "assistant", "content": "Prior task complete."},
        ]
        ...
        with (
            ...
            result = agent.run_conversation(
                "Execute the new browser task now.",
                conversation_history=history,
            )

        assert result["final_response"] == "Starting the requested browser task."
        assert result["api_calls"] == 2
