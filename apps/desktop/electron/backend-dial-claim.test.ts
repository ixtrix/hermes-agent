...
describe('main.ts wiring for #90812', () => {
  ...
  it('routes every registry-scoped REST dispatch (hermes:api) through the single-owner claim', () => {
    ...
    expect(handlerStart).toBeGreaterThan(-1)
    const handlerEnd = mainSource.indexOf('\nfunction registryConnectionKind(', handlerStart)
    expect(handlerEnd).toBeGreaterThan(handlerStart)
    const body = mainSource.slice(handlerStart, handlerEnd)

    expect(body).toContain('backendDialClaims.run(claimKey')
    expect(body).toContain("ensureRegistryBackend(registryConnectionId, routeProfile, '', expectedRevision)")
  })

  it('validates fixed-route revisions after a coalesced claim and again at fetch', () => {
    const handlerStart = mainSource.indexOf('async function dispatchRegistryApiRequest(')
    expect(handlerStart).toBeGreaterThan(-1)
    const handlerEnd = mainSource.indexOf('\nfunction registryConnectionKind(', handlerStart)
    expect(handlerEnd).toBeGreaterThan(handlerStart)
    const body = mainSource.slice(handlerStart, handlerEnd)
    const claim = body.indexOf('await backendDialClaims.run(')
    const resolvedGuard = body.indexOf('assertCurrentRegistryConnectionRevision(', claim)
    const fetch = body.indexOf('await fetchJsonForBackend(', resolvedGuard)
    const finalGuard = body.indexOf('beforeFetch:', fetch)
    const beforeFetchGuard = body.indexOf('assertCurrentRegistryConnectionRevision(', finalGuard)
    const responseGuard = body.indexOf('assertCurrentRegistryConnectionRevision(', beforeFetchGuard + 1)

    expect(claim).toBeGreaterThan(-1)
    expect(resolvedGuard).toBeGreaterThan(claim)
    expect(fetch).toBeGreaterThan(resolvedGuard)
    expect(finalGuard).toBeGreaterThan(fetch)
    expect(beforeFetchGuard).toBeGreaterThan(finalGuard)
    expect(responseGuard).toBeGreaterThan(beforeFetchGuard)
    expect(body).toContain("ensureRegistryBackend(registryConnectionId, routeProfile, '', expectedRevision)")
  })

  it('checks fixed-route authority inside every token retry and blocks unguarded OAuth redirects', () => {
    const fetchStart = mainSource.indexOf('function fetchJson(url, token, options: any = {})')
    const fetchEnd = mainSource.indexOf('\nfunction downloadViaTokenToFile(', fetchStart)
    const fetchBody = mainSource.slice(fetchStart, fetchEnd)
    const retry = fetchBody.indexOf('return withRetry(')
    const guard = fetchBody.indexOf('options.beforeFetch?.()', retry)
    const request = fetchBody.indexOf('client.request(', guard)

    expect(retry).toBeGreaterThan(-1)
    expect(guard).toBeGreaterThan(retry)
    expect(request).toBeGreaterThan(guard)

    const oauthStart = mainSource.indexOf('function fetchJsonViaOauthSession(url, options: any = {})')
    const oauthEnd = mainSource.indexOf('\n// ---------------------------------------------------------------------------', oauthStart)
    const oauthBody = mainSource.slice(oauthStart, oauthEnd)
    const oauthGuard = oauthBody.indexOf('options.beforeFetch?.()')
    const oauthRequest = oauthBody.indexOf('electronNet.request(')

    expect(oauthGuard).toBeGreaterThan(-1)
    expect(oauthRequest).toBeGreaterThan(oauthGuard)
    expect(oauthBody).toContain("redirect: options.beforeFetch ? 'error' : 'follow'")
  })
})
