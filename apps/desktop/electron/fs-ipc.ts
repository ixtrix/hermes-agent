...
export interface FsIpcDeps {
  authorizeWorkstationFolders: (webContentsId: number) => Promise<boolean>
  hermesHome: string
...
export function registerFsIpc({
  ...
  ipcMain.handle('hermes:workstationFolders', async (event, op, payload) => {
    if (!(await authorizeWorkstationFolders(event.sender.id))) {
      throw new Error('Workstation folders are available only for the enrolled Scope Hermes Internal gateway')
    }

    return workstationFolders(op, payload)
  })
