...
async def auth_native_authorize(
    ...
    # Managed builds bind native login to their configured OIDC provider.
    # Otherwise an empty provider selects the single brokerable session
    # provider, excluding password fallback providers from the candidate count.
    provider_name = provider or (
        managed_oidc["provider"] if managed_oidc is not None else ""
    )
    p = get_provider(provider_name) if provider_name else None
