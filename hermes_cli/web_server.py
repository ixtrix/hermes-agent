...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        if internal:
            try:
                _set_ws_principal(ws, consume_internal_credential(internal))
        ...
        try:
            _set_ws_principal(ws, _verify_ws_ticket_session(consume_ticket(ticket)))
            return None, "ticket"
        except TicketInvalid as exc:
            audit_log(
                AuditEvent.WS_TICKET_REJECTED,
...
def start_server(
    ...
    app.state.auth_required = managed_staff or should_require_auth(host)
